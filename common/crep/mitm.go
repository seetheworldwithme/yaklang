...
const (
	defaultMITMRootCAName   = "Sentinel MITM Root CA"
	defaultMITMGMRootCAName = "Sentinel MITM GM Root CA"
)
...
func InitMITMCert() {
	...
	// Certificates generated by earlier builds exposed the former product name.
	// Regenerate only those legacy certificates so upgrades also download the new
	// branded root CA instead of continuing to reuse the file on disk.
	if isLegacyMITMCertificate(defaultCA) {
	...
	if defaultCA == nil || defaultKey == nil {
		var err error
		defaultCA, defaultKey, err = tlsutils.GenerateSelfSignedCertKeyWithCommonName(defaultMITMRootCAName, "mitmserver", nil, nil)
		if err != nil {
	...
	if defaultGMCA == nil || defaultGMKey == nil {
		var err error
		defaultGMCA, defaultGMKey, err = tlsutils.GenerateGMSelfSignedCertKey(defaultMITMGMRootCAName)
		if err != nil {
...
func isLegacyMITMCertificate(certRaw []byte) bool {
	...
	subject := strings.Join(append(cert.Subject.Organization, cert.Subject.CommonName), " ")
	return strings.Contains(strings.ToLower(subject), "yakit")
}
